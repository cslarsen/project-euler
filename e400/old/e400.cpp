...
static map<Z, bool> loss; // for sub-trees in for-loop
static map<Z, bool> iswin; // for trees at winner() entry
...
static player winner(const tree& t, const player current)
{
  ...
  /*
   * Before trying all possibilities, see if this exact tree has been
   * processed before, and just return the result.
   */
  const Z th = tree_hash(t);
  if ( iswin.find(th) != iswin.end() )
    return iswin[th]? current : other(current);

  /*
   * First, see if we can find a node that leads to a
   * guaranteed win.
   */
  for ( Z n=2; n<size; ++n ) {
    tree c(t);
  ...
  /*
   * TODO:
   * We need to be smarter about this. If there is a know
   * LOSING tree such as T(7), can we remove a node that
   * gives this tree?
   *
   * Further, if we know that T(k-1) is a WINNING tree,
   * we need to avoid those, because they lead to a
